...
namespace Epam.GraphQL.Configuration.Implementations.Fields.ExpressionFields
{
    internal class NullableExpressionField<TEntity, TReturnType, TExecutionContext> : ExpressionField<TEntity, TReturnType?, TExecutionContext>
        ...
        where TEntity : class
    {
        public NullableExpressionField(BaseObjectGraphTypeConfigurator<TEntity, TExecutionContext> parent, Expression<Func<TEntity, TReturnType?>> expression, string? name)
            : base(parent, expression, name)
        {
        }

        public NullableExpressionField(BaseObjectGraphTypeConfigurator<TEntity, TExecutionContext> parent, Expression<Func<TExecutionContext, TEntity, TReturnType?>> expression, string name)
            : base(parent, expression, name)
        {
        }
