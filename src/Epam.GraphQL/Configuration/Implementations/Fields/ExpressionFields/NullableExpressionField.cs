...
namespace Epam.GraphQL.Configuration.Implementations.Fields.ExpressionFields
{
        ...
        where TEntity : class
    {
        ...
        public NullableExpressionField(RelationRegistry<TExecutionContext> registry, BaseObjectGraphTypeConfigurator<TEntity, TExecutionContext> parent, Expression<Func<TExecutionContext, TEntity, TReturnType?>> expression, string name)
            : base(registry, parent, expression, name)
        {
        }
